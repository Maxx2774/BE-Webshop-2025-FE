...
//         card.classList.add("card", "mb-2", "p-2", "d-flex", "flex-row", "align-items-center", "gap-2");
...
//         textDiv.innerHTML = `<p class='mb-1 fw-bold'>${product.name}</p><p class='text-danger fw-semibold'>${new Intl.NumberFormat('sv-SE', { style: 'currency', currency: 'SEK' }).format(product.price)}</p>`;
...
//         quantityDiv.append(minusBtn, quantitySpan, plusBtn);
...
// Uppdaterad funktion för att rendera varukorgen
// const renderCart = () => {
//     const cartContainer = document.getElementById('cart-container');
//     cartContainer.innerHTML = "";
//     let cart = JSON.parse(localStorage.getItem('cart')) || [];
...
//     // Display total price and add delivery cost
//     const deliveryCost = 50;  // Delivery cost
//     const totalWithDelivery = totalPrice + deliveryCost;

//     document.getElementById('total-products').textContent = cart.length;
...
//     document.getElementById('delivery-cost').textContent = new Intl.NumberFormat('sv-SE', { style: 'currency', currency: 'SEK' }).format(deliveryCost);
//     document.getElementById('total-with-delivery').textContent = new Intl.NumberFormat('sv-SE', { style: 'currency', currency: 'SEK' }).format(totalWithDelivery);
...
//     cart = cart.filter(item => item.id !== productId);
//     localStorage.setItem('cart', JSON.stringify(cart));
//     renderCart(); // Update cart display after removal
// };
...
const renderCart = () => {
    ...
    cartSummaryDiv.classList.add("w-25", "bg-light", "p-3", "rounded", "shadow-sm");
    cartSummaryDiv.style.height ="300px";
    cartSummaryDiv.innerHTML = `
        <h5 class="fw-bold">Orderöversikt</h5>
        <p><strong>Antal produkter:</strong> <span id="total-products">${cart.length}</span></p>
        <p><strong>Totalpris varor:</strong> <span id="total-price">${new Intl.NumberFormat('sv-SE', { style: 'currency', currency: 'SEK' }).format(totalPrice)}</span></p>
        <p><strong>Leveranskostnad:</strong> <span id="delivery-cost">${new Intl.NumberFormat('sv-SE', { style: 'currency', currency: 'SEK' }).format(deliveryCost)}</span></p>
        <hr>
        <h5><strong>Totalt att betala:</strong> <span id="total-with-delivery">${new Intl.NumberFormat('sv-SE', { style: 'currency', currency: 'SEK' }).format(totalPrice + deliveryCost)}</span></h5>
    `;
