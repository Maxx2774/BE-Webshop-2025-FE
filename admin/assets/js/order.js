import { verifyToken, baseUrl, loggedUser, logOutUser, adminCheck, GetAsync } from './services.js';
...
document.getElementById("orderDetailsModal").addEventListener("show.bs.modal", async (event) => {
    ...
    document.getElementById("order-status").innerHTML = `<span class="fw-semibold">Status:</span> ${orderStatus(order.data.status, "orderStatus")}`;
    ...
    document.getElementById("payment-status").innerHTML = `<span class="fw-semibold">Status:</span> ${orderStatus(order.data.payment_status, "paymentStatus")}`;
